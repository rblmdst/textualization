...
function load(id, translations, lang) {
  var method =
    typeof translations === "string" ? loadTranslationsPath : loadTranslations;
...
function loadTranslationsPath(id, path, reload) {
  ...
  for (var i = 0, l = languages.length; i < l; i++) {
    ...
    var translations;
    try {
      translations = fs.readFileSync(paths.join(path, lang + ".js"), {
        encoding: "utf8"
      });
    } catch (error) {
      continue;
    ...
    try {
      ...
      if (!jshint(translations)) {
        ...
        for (var e in checkfail) {
          // array [{id:'(error)|(main)...',raw,code,evidence,line,character,scope:'(main)',a,b,c,d,reason},...]
          if (checkfail[e]) {
            // Fix null result at end
            jsHintError.stack +=
              "\tLine " +
              checkfail[e].line +
              " > " +
...
function translate(text, params, lang) {
  ...
  if (!heap[id] || Object.keys(heap[id]).length === 0) {
    return original;
  }
  ...
  try {
    ...
    for (i = 1, l = languages.length; i < l; i++) {
      // Search any translation by priority order.
    ...
    search =
      'heap["' +
      [id, Object.keys(heap[id])[0]].concat(node).join('"]["') +
      '"]';
    try {
      match = eval(search);
    } catch (error) {
  ...
  try {
    if (typeof match === "string") {
      // Simple printf translation
      return match.format(params);
    } else if (Array.isArray(match)) {
      // Grammatical number translation
      var _num_;
      ...
      if (_num_ >= 0) {
        if (match.length == 2) {
          ...
          return _num_ >= match.length - 1
            ? match[match.length - 1].format(params)
            : match[_num_].format(params);
        }
      } else {
        throw new Error("Invalid params for gramatical number translation");
      }
    } else if (typeof match === "function") {
      // Function handled translation
      func = functionBody(match);
      func =
        "function(" + Object.keys(params).join(",") + "){\n" + func + "\n}";
      eval("func = " + func + ";");
      var funcparams = [];
